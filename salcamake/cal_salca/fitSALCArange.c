...
void writeParams(dataStruct *data,control *dimage)
{
  ...
  fprintf(opoo,"m %f\nc %f\nk %f\na %f\nk2 %f\nk3 %f\n",data->m,data->c,data->k,data->a,data->k2,data->k3);
  RMSE=0.0;
  for(i=0;i<data->numb;i++){
    DN=calibrate(data->r[i],data->rho[i],data->m,data->c,data->k,data->a,data->k2,data->k3,data->P0[i]);
...
mp_par *fitBounds(int nPars,control *dimage)
{
  ...
  //mparS[0].side=3;
  //mparS[0].limited[0]=1;
  //mparS[0].limits[0]=50.0;
  //mparS[0].limited[1]=1;
  //mparS[0].limits[1]=1000.0;
  //mparS[0].step=10.0;
  /*c*/
  mparS[1].fixed=1;
  //mparS[1].side=3;
  //mparS[1].limited[0]=1;
  //mparS[1].limits[0]=15.0;
  //mparS[1].limited[1]=1;
  //mparS[1].limits[1]=500.0;
  //mparS[1].step=5.0;
  /*k*/
  mparS[2].fixed=1;
  //mparS[2].side=3;
  //mparS[2].limited[0]=1;
  //mparS[2].limits[0]=0.05;
  //mparS[2].limited[1]=1;
  //mparS[2].limits[1]=100.0;
  //mparS[2].step=1.0;
  /*a*/
  mparS[3].fixed=0;
  mparS[3].side=3;
  ...
  mparS[4].side=3;
  mparS[4].limited[0]=1;
  mparS[4].limits[0]=0.01;
  mparS[4].limited[1]=1;
  mparS[4].limits[1]=50.0;
  mparS[4].step=0.5;
  /*k3*/
  mparS[5].fixed=0;
  mparS[5].side=3;
  mparS[5].limited[0]=1;
  mparS[5].limits[0]=0.01;
  mparS[5].limited[1]=1;
...
int fitError(int numb,int npar,double *pars,double *deviates,double **derivs,void *private)
{
  int i=0;
  float DN=0;
  float m=0,c=0,k=0,a=0,k2=0,k3=0,P0=0;
  float calibrate(float,float,float,float,float,float,float,float,float);
  float x=0,arg=0,expo=0;
  float arg2=0,expo2=0;
  ...
  for(i=0;i<numb;i++){
    P0=data->P0r[i]/salcaRange(a,k2,k3,8.0);
    data->P0[i]=P0;
    ...
    if(derivs){ /*all negative as it's DN(truth)-DN(model)*/
      x=data->rho[i]*P0*salcaRange(a,k2,k3,data->r[i]);
      arg=k*x;
      ...
      /*range parameters*/
      arg2=k2*pow(data->r[i],k3);
      if(arg2<730.0)expo2=1.0-exp(-1.0*arg2);
      else          expo2=1.0;
      ...
      if(derivs[4]){  /*k2*/
        if(arg2<730.0)dfR=pow(data->r[i],k3-a)*exp(-1.0*arg2);
        else          dfR=0.0;
        derivs[4][i]=-1.0*dfR*dDNdfR*data->weight[i];
      }

      if(derivs[5]){  /*k3*/
        if(arg2<730.0)dfR=k2*pow(data->r[i],k3-a)*log(data->r[i])*exp(-1.0*arg2);
        else          dfR=0.0;
