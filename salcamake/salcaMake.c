...
int main(int argc,char **argv)
{
  ...
  /*if smoothing, set up smoothing array*/
  if(options->fun->sWidth>TOL){
    setSmoother(options->fun);
  }
...
void squint(float *cZen,float *cAz,float zM,float aM,float zE,float aE,float omega)
{
  ...
  /*rotate about y axis*/
  /*}else{  BEWARE, there is aisgn issue somewhere in the below. It seems to work as is.
    thetaZ=-1.0*atan2(rX,rY);
...
void setSmoother(funcFit *fun)
{
  ...
  minY=0.0001;  /*point to stop counting*/
  ...
  if(fun->sWidth<TOL){    /*no smoothing, just denoise*/
    ...
    for(i=0;i<fun->nLength;i++){
      ...
      for(j=0;j<fun->sLength;j++){
        place=(int)(((float)i+(float)(j-fun->sLength/2))*fun->sRes/res+0.5);
        if((place>=0)&&(place<length)){
          smoothed[i]+=((float)data[place]-(float)backSig)*fun->smoother[j];
...
/*for(i=0;i<fun->nLength;i++){
  fprintf(stdout,"%f %f",(float)i*fun->sRes,smoothed[i]);
  if(i<length)fprintf(stdout," %f %d",(float)i*res,data[i]);
  fprintf(stdout,"\n");
}
exit(1);*/
...
void trackSmoothed(float *smoothed,int place,control *options,float *range,float *integral,float start)
{
  ...
  *integral*=options->fun->sRes/0.15;
