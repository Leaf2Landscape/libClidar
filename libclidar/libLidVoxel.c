...
void silhouetteImage(int nFiles,pCloudStruct **alsData,tlsScan *tlsData,rImageStruct *rImage,lidVoxPar *lidPar,int *voxList,int nIn,tlsVoxMap *map)
{
  ...
  float zen=0,az=0;
  double vect[3];
  void markPointSilhouette(double *,rImageStruct *,int,lidVoxPar *,float,float,double);
...
void markPointSilhouette(double *coord,rImageStruct *rImage,int bin,lidVoxPar *lidPar,float gap,float refl,double r)
{
...
float tlsPointSize(double range,float refl,float tanDiv,float beamRad,float min,float max,float rhoApp,float gap)
{
  float d=0;
  float appRefl=0;
  float reflScale=0;
...
void makeBinImage(double *coord,float *area,float *count,char *rImage,int numb,float gap,double r,float refl,float beamRad,int rNx,int rNy,float rimRes,lidVoxPar *tlsPar)
{
  int xInd=0,yInd=0,rPlace=0;
  int xStart=0,xEnd=0;
  int yStart=0,yEnd=0;
